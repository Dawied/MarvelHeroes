...
In het starter project zit een volledige applicatie om Marvel Heroes en hun Abilities op te voeren, te wijzigen en te verwijderen.
...
2. Open de project folder in je IDE, open een terminal en installer de depencies

``` composer install ```

``` npm install ```

3. Rename example.env naar .env en edit:
...
6. Start de applicatie en open in je browser op http://localhost:8000

``` composer run dev ```

Pak de [tutorial](https://www.sdcheatsheets.nl/koppeltabellen-in-laravel) erbij en volg de stappen een voor een.
